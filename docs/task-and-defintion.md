...
When I started learning ECS Task Definitions, I came across two fields: Task Execution Role and Task Role. I was confused about why ECS needs two roles and what each one does.

What problem do these two roles solve?

* Task Execution Role: used by ECS service itself (AWS-managed)
...
The Task Execution role is assumed by the ECS service/agent before your container starts. ECS needs permission to:
...
For this project, I will use the AWS managed policy called `AmazonECSTaskExecutionRolePolicy`.
...
```     
5. Because we are storing the secrets in AWS Parameter store, we need to manually add the permissions as a policy to the task execution role. Create a file named `ssm-read-policy.json` that contains the parameter store permissions specific to the path where you've stored all the parameters:
...
**Verification**
1. Confirm the role exists and is active:
```
$ aws iam get-role --role-name CruddurServiceExecutionRole
```
2. Verify the managed execution policy is attached:
```
$ aws iam list-attached-role-policies --role-name CruddurServiceExecutionRole
```
3. Verify the inline SSM policy is present:
```
$ aws iam list-role-policies --role-name CruddurServiceExecutionRole
```
...
The task role is assumed by the application container at runtime to access AWS services such as:
...
2. Create a file named `ssm-access-policy.json` that contains the Session Manager permissions:
```
{
    "Version":"2012-10-17",		 	 	 
    "Statement": [
...
1. Confirm the role exists and is active:
```
$ aws iam get-role --role-name CruddurTaskRole
```
2. Verify the managed CloudWatch policy is attached:
```
$ aws iam list-attached-role-policies --role-name CruddurTaskRole
```
3. Verify the inline SSM access policy is present:
```
$ aws iam list-role-policies --role-name CruddurTaskRole
```
